    ...
    for fold in folds:
        ...
        for ckp_path in ckpt_paths:
            ...
            df = pd.DataFrame({
                ...
                #"Logits": [row.tolist() for row in logits_np],
                "Probabilities": [row.tolist() for row in probs],
            })

            out_path = os.path.join(cfg.exp_dir, f"test_classification_predictions_f{fold}_{ckpt_name}.csv")
            ...
            metrics_out_path = os.path.join(cfg.exp_dir, f"test_classification_metrics_f{fold}_{ckpt_name}.csv")
